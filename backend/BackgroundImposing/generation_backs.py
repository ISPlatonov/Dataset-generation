...
def get_approx(black_and_white_mask, detail_name):
    """
...
def generate_new_background(detail_num, img, mask_gray, background1, sdvig_x, sdvig_y):
    """
    ...
    background1, gt = adding_img_on_background(img, mask_gray, background1, sdvig_x, sdvig_y)
    return background1, gt

def adding_img_on_background(img, mask_gray, background, sdvig_x, sdvig_y):
    """
...
def get_mask(mask_gray, sdvig_x, sdvig_y):
    """
...
def get_shifts(background, mask_gray):
    """
...
def check_iou(d, rect1, detail_num):
    """
    Проверка IoU.
    :param d: dictionary
    :param rect1: array
    ...
    for i in range(detail_num):
        score = get_score(d["annotations"][i]["yolo"], rect1)
        if score > 0.2:
            flag = 0
            break
        score = get_score(rect1, d["annotations"][i]["yolo"])
        if score > 0.2:
            flag = 0
            break
    return flag

def add_detail_on_background(img, mask_gray, background, sdvig_x, sdvig_y):
    """
    ...
    :return: array, array
    """
    cur_mask = get_mask(mask_gray, sdvig_x, sdvig_y)
    background, _ = adding_img_on_background(img, mask_gray, background, sdvig_x, sdvig_y)
    return background, cur_mask

def generate_new_photo(detail_num, id, detail_name, img, path_detail, path_mask, background, masks_array,
    ...
    if detail_num <= 0:
        ...
        img, masks_array[:, :, 0] = generate_new_background(detail_num, detail, mask, background, sdvig_x, sdvig_y)
    else:
        flag = check_iou(d, rect, detail_num)
        if flag == 1:
            d = writing_in_json(detail_num, img, detail_name, id, rect, count, d)
            img, masks_array[:, :, detail_num] = add_detail_on_background(detail, mask, img, sdvig_x, sdvig_y)
        else:
            return img, masks_array, d, detail_num
...
all_details_names = ("batman", "bolshoy_grover", "bolshoy_podshipnik", "dvernoye_koltso", "grib", "koltso",
                     ...
                     "vint", "vtulka", "hand")
print(os.getcwd() + "\n")
output = open(f'D:/output.txt', 'w+')
name_back = "./backgrounds/" + str(1) + ".jpg"
...
for id in range(10000):
    ...
    name_back = "./backgrounds/" + str(k) + ".jpg"
    ...
    a = int(random.uniform(1, len(all_details_names)))  # количество деталей на картинке
